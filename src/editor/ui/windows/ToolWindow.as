package editor.ui.windows
{
    ...
    import editor.ui.elements.ToolButton;
    ...
    public class ToolWindow extends Window
    {
        ...
        [Embed(source='../../../../assets/tool_rand.png')]
        ...
        private const TILE_TOOLS:Array         = [ToolTilePlace, ToolTileEyedrop, ToolTileRectangle, ToolTileFill, ToolTileRandom];
        private const TILE_IMAGE:Array        = [ImgPaint, ImgEyedrop, ImgFillRect, ImgFill, ImgRandom];
            ...
            if (!e.ctrlKey)
            {
                for (var i:int = 0; i < tools.length; i++)
                {
                    if (e.keyCode == String("1").charCodeAt() + i)
                        Ogmo.level.selectedLayer.setTool(new tools[i](Ogmo.level.selectedLayer));
                }
            }
        }
